					...
					<mm_body>
						<mm_form class="bar_filter">
							...
							<mm_list :col="3">
								...
								<!--{/loop}-->
								<mm_col>
									<mm_btn class="btn_primary-x" type="reset" @click.native="reset();search()">重置
									</mm_btn>
								</mm_col>
							</mm_list>
						</mm_form>
						...
						<mm_table type="2">
							<thead>
								<tr>
									<th scope="col" class="th_selected"><input type="checkbox" :checked="select_state"
											@click="select_all()" /></th>
									<th scope="col" class="th_id"><span>#</span></th>
									<!--{loop field v idx}-->
									<!--{if(idx > 0)}-->
									<th scope="col">
										<control_reverse title="${v.title}" v-model="query.orderby" field="${v.name}"
							...
							<tbody>
								<tr v-for="(o, idx) in list" :key="idx">
									<th scope="row"><input type="checkbox" :checked="select_has(o[field])"
											@click="select_change(o[field])" /></th>
									<th scope="row"><span>{{ o[field] }}</span></th>
									<!--{loop field v idx}-->
									<!--{if(v.format)}-->
									<!--{if(v.format.table)}-->
									<td><span>{{ $get_name(${v.label}, o.${v.format.key}, '${v.format.id || v.format.key}', '${v.format.name}') }}</span>
									</td>
									<!--{else}-->
									...
									<!--{/loop}-->
									<td>
										<mm_btn class="btn_primary" :url="'./${name}_form?${sql.key}=' + o[field]">修改
										</mm_btn>
	...
	export default {
		...
		data() {
			return {
				...
				/*[loop js.data v idx]*/
				// ${' ' + v.title}
				'${v.name}': [ /*[loop v.value a idx]*/ /*[if idx == 0]*/ '${a}' /*[else]*/ ,
					'${a}' /*[/if]*/ /*[/loop]*/ ],
				/*[/loop]*/
				// 视图模型
				vm: {}
